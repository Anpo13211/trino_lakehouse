"""
Create Iceberg tables in Trino and link Parquet files from MinIO
"""
...
def get_trino_container_name(use_sudo: bool = False):
    """
    Auto-detect Trino container name
    """
    command = []
    if use_sudo:
        command.append("sudo")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create Iceberg tables in Trino and link Parquet files from MinIO"
    )
    parser.add_argument(
        "dataset",
        type=str,
    ...
    print("="*60)
    print("Iceberg Table Creator & Parquet Linker")
    print("="*60)
